...
# Find all visible dialog-like elements
result = page.evaluate("""() => {
    ...
    for(var i=0;i<all.length;i++) {
        ...
        if(cls.includes('dialog') || cls.includes('modal') || cls.includes('overlay') || cls.includes('mask') || cls.includes('popup')) {
            ...
            if(rect.width > 5 && rect.height > 5 && all[i].offsetParent !== null) {
                var btns = [];
                ...
                found.push({
                    ...
                    text: all[i].textContent.substring(0,80).replace(/\\n/g,' '),
                    btns: btns
                });
            }
        }
...
print('Visible dialog elements:', flush=True)
parsed = json.loads(result)
for d in parsed:
    print('  class=' + d['cls'][:50] + ' rect=' + d['rect'] + ' btns=' + str(d['btns']) + ' text=' + d['text'][:50], flush=True)
if not parsed:
