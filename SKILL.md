...
- **读取 `--JSON--` 输出**，解析 `pass_products` 数组，用表格格式回复用户：
  ```
  ✅ 通过：3件
    - [童裝] 主货号(637704645844) 男童短袖套装...
    - [童裝] 主货号(758357471915) 棉家居服...
  ❌ 拒绝：0件

  分配结果：
    - 順順の小屋童裝（本土）← 2件（已认领）
    - 吉象星連坊（本土）← 0件
    - 未匹配：1件（食品→安吉白茶，留在采集箱）
  ```
...
- 店铺名必须与ERP弹窗**完全一致**
- 不在映射表中的类目 → 留在采集箱，不删不认领
- 类目由LLM识别：童裝/五金/百货/3C/服装/食品/美妆/家居/母婴/户外/宠物/其他
...
- `1到小屋，2到吉象星連坊` → 按序号分
- `食品类的到吉象星連坊` → 按类目分
- `主货号(529363575764)到小屋` → 含ID分

**你收到回复后**：
```powershell
# 按分配结果：
$env:PYTHONIOENCODING='utf-8'; python run_workflow.py --claim-to "順順の小屋童裝（本土）" --publish --products 637704645844,758357471915
$env:PYTHONIOENCODING='utf-8'; python run_workflow.py --claim-to "吉象星連坊（本土）" --publish --products 529363575764
```
...
| `run_workflow.py --add-rule "category:童裝->順順の小屋童裝（本土）"` | 动态加分配规则 |
